  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="Name">
        <template slot-scope="scope">
          {{ scope.row.name }}<i class="el-icon-edit" @click="openPlanDetail(scope.row.id)"></i>
        </template>
      </el-table-column>
      ...
      >
        <template>
          <el-form ref="form" :model="form" label-width="80px" style="padding: 20px">
            ...
            <el-form-item>
              <el-button v-if="id != ''" :loading="formLoading" type="primary" @click="addPlan">立即创建</el-button>
              <el-button v-else :loading="formLoading" type="primary" @click="addPlan">立即创建</el-button>
...
<script>
import { getTree, addPlan, getOnePlan } from '@/api/plan'
  ...
  methods: {
    ...
    openPlanDetail(id) {
      getOnePlan(id).then((response)=>{
        ...
        this.form.parent = response.data.parent != "" ? [response.data.parent] : []
